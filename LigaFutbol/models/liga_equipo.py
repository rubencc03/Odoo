...
class LigaEquipo(models.Model):
    ...
    def _compute_puntos(self):
        for record in self:
            if(record.goles_a_favor-record.goles_en_contra >3):
                record.puntos=record.victorias * 3 + record.empates +1
            elif(record.goles_a_favor-record.goles_en_contra <-3):record.puntos = record.victorias * 3 + record.empates -1
            else:record.puntos = record.victorias * 3 + record.empates
